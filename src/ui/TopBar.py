...
class TopBar(QWidget):
    ...
    async def export_txt(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export as TXT", "", "Text files (*.txt)"
        )
        if path:
            try:
                await asyncio.to_thread(
                    SubtitleGenerator.to_txt, self.subtitles_manager._subtitles, path
                )
                QMessageBox.information(
                    self, "Export Successful", f"Subtitles exported as TXT:\n{path}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export TXT:\n{str(e)}"
                )
    ...
    async def export_srt(self):
        ...
        """
        path, _ = QFileDialog.getSaveFileName(
            self, "Export as SRT", "", "SRT files (*.srt)"
        )
        if path:
            try:
                await asyncio.to_thread(
                    SubtitleGenerator.to_srt, self.subtitles_manager._subtitles, path
                )
                QMessageBox.information(
                    self, "Export Successful", f"Subtitles exported as SRT:\n{path}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export SRT:\n{str(e)}"
                )
    ...
    async def export_ass(self):
        ...
        """
        path, _ = QFileDialog.getSaveFileName(
            self, "Export as ASS", "", "ASS files (*.ass)"
        )
        if path:
            try:
                await asyncio.to_thread(
                ...
                QMessageBox.information(
                    self, "Export Successful", f"Subtitles exported as ASS:\n{path}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export ASS:\n{str(e)}"
                )
    ...
    async def export_mp4(self):
        ...
        """
        path, _ = QFileDialog.getSaveFileName(
            self, "Export as MP4", "", "MP4 files (*.mp4)"
        )
        if path:
            try:
                ...
                QMessageBox.information(
                    self, "Export Successful", f"Video exported with subtitles:\n{path}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export MP4:\n{str(e)}"
                )
    ...
    def reset_style_to_default(self):
        ...
        QMessageBox.information(
            self, "Style Reset", "Style has been reset to the default configuration."
        )
    ...
    async def save_style_to_file(self):
        ...
        """
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Style", str(STYLES_DIR), "JSON files (*.json)"
        )
        if path:
            try:
                ...
                QMessageBox.critical(
                    self, "Save Error", f"Failed to save style:\n{str(e)}"
                )
    ...
    async def load_style_from_file(self):
        ...
        """
        path, _ = QFileDialog.getOpenFileName(
            self, "Load Style", str(STYLES_DIR), "JSON files (*.json)"
        )
        if path:
            try:
                await asyncio.to_thread(self.style_manager.load_from_file, path)
                QMessageBox.information(
                    self, "Style Loaded", f"Style loaded from:\n{path}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Load Error", f"Failed to load style:\n{str(e)}"
                )
